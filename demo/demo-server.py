...
def make_auth_headers():
    return {'Authorization': 'basic {}'.format(SIGNUP_AUTH_SECRET)}
...
def get_emails(count=20, offset=0):
    ...
    """
    try:
        url = SIGNUP_URL + "?count={}&offset={}".format(count,offset)
        resp = requests.get(SIGNUP_URL, headers=make_auth_headers())
        emails = resp.json()
...
def refresh_emails():
    ...
    Return {'status': True, 'new': new emails, 'expired': expired emails}
    """
    global EMAILS
    ...
    return {'status': True, 'new': new, 'expired': expired}
...
def make_provision_plan( new_users, expired_users ):
    ...
    * delete the volumes and gateways and users for the expired users
    ...
    provision_create_volumes = [{
        'name': 'demo.volume-{}'.format(user_name),
        'owner': user_name,
    ...
    provision_create_ug_gateways = [{
        'name': 'demo.gateway-{}'.format(user_name),
        'volume': 'demo.volume-{}'.format(user_name),
        'type': 'UG',
        'host': 'localhost',
        'port': UG_PORT,
    } for user_name in new_users]
...
def run_provision_plan(provision_plan):
    ...
    p = subprocess.Popen(["syndicate-amd-server", "provision"])
    out, err = p.communicate(provision_plan_txt)
    p.wait()

    if p.returncode != 0:
        log.error("Failed to run `syndicate-amd-server provision`")
...
def upload_keys(provision_plan):
    """
    ...
    for user_info in provision_plan['users']['create']:
    ...
    for volume_info in provision_plan['volumes']['create']:
        volume_name = volume_info['name']
        volume_owner = volume_info['owner']
        volume_pkey = storage.load_private_key( syndicate_config, 'volume', volume_name )
        if volume_pkey is None:
            log.error("Automount daemon failed to produce key for {}".foramt(volume_name))
            return False

        pkeys[user_name]['volume_pkey'] = volume_pkey

    for user_name in pkeys.keys():
        # encrypt keys with password 
        user_info = EMAILS[user_name]
        user_password = user_info['password']

        f = Fernet(user_password)
        pkeys[user_name]['user_pkey'] = f.encrypt(pkeys[user_name]['user_pkey'])
        pkeys[user_name]['volume_pkey'] = f.encrypt(pkeys[user_name]['volume_pkey'])
    ...
    for user_name in pkeys.keys():
        ...
        try:
            ...
            req = requests.post(SIGNUP_URL + '/provision/{}'.format(urllib.quote(user_name)), headers=make_auth_headers(), 
                    data={'user_private_key': pkeys[user_name]['user_pkey'], 'volume_private_key': pkeys[user_name]['volume_pkey']})
