...
for NAME in $RANDOM_UG_GATEWAY_NAME $RANDOM_RG_GATEWAY_NAME $RANDOM_AG_GATEWAY_NAME; do
   ...
   # should succeed
   $SYNDICATE_TOOL -c "$CONFIG_PATH" create_gateway email="$RANDOM_USER_NAME" volume="$RANDOM_VOLUME_NAME" name="$GW_NAME" private_key=auto type="$GW_TYPE"
   RC=$?
   ...
   $SYNDICATE_TOOL -c "$CONFIG_PATH" -u "$RANDOM_USER_NAME" update_gateway private_key="$CONFIG_DIR/new_key.pem" host="newhost" port="45678" name="$GW_NAME" 
   RC=$?
   ...
   # check 
   GATEWAY_JSON="$($SYNDICATE_TOOL -c "$CONFIG_PATH" read_gateway "$GW_NAME")"
   RC=$?
