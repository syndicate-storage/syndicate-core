...
if __name__ == "__main__":
    ...
    for i in xrange(2, 5):
        ...
        output_path = "/dir1/dir%s/%s" % (i, os.path.basename(local_path))
        local_fd = open(local_path, "r")
        expected_data = local_fd.read()
        local_fd.close()

        # refresh directory 
        exitcode, out = testlib.run( REFRESH_PATH, '-d2', '-f', '-c', os.path.join(config_dir, 'syndicate.conf'),
        ...
        if exitcode != 0:
            ...
            raise Exception("Failed to refresh '%s'" % os.path.dirname(output_path))

        # wait for AG to process
        time.sleep(10)
    ...
    for p in ['/', '/dir1', '/dir1/dir2', '/dir1/dir3', '/dir1/dir4', output_path]:
