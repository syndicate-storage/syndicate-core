...
if __name__ == "__main__":
    ...
    for rep in xrange(0, NUM_REPS):
        ...
        # delete them all, but in random order 
        random.shuffle(expected_paths)
        exitcode, out = testlib.run( UNLINK_PATH, "-d2", '-f', '-c', os.path.join(config_dir, 'syndicate.conf'), '-u', testconf.SYNDICATE_ADMIN, '-v', volume_name, '-g', gateway_name, *expected_paths)
        testlib.save_output( output_dir, "syndicate-unlink-%s" % rep, out )
        if exitcode != 0:
            stop_and_save( output_dir, rg_proc, rg_out_path, "syndicate-rg")
