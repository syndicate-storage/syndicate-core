...
int md_verify_signature_raw( EVP_PKEY* public_key, char const* data, size_t len, char* sig_bin, size_t sig_bin_len ) {
   ...
   if( rc <= 0 ) {
      ...
      SG_error("EVP_DigestVerifyInit_ex( %p ) rc = %d, error_code = %ld\n", public_key, rc, ERR_get_error() );
      md_openssl_error();
   ...
   if( rc <= 0 ) {
      ...
      SG_error("EVP_PKEY_CTX_set_rsa_padding rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EINVAL;
   }
   ...
   if( rc <= 0 ) {
      ...
      SG_error("EVP_PKEY_CTX_set_rsa_pss_saltlen rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EINVAL;
   }
   ...
   if( rc <= 0 ) {
      ...
      SG_error("EVP_DigestVerifyUpdate rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EINVAL;
   }
   ...
   if( rc <= 0 ) {
      ...
      SG_error("EVP_DigestVerifyFinal rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EBADMSG;
   }
...
int md_sign_message_raw( EVP_PKEY* pkey, char const* data, size_t len, char** sig, size_t* siglen ) {
   ...
   if( rc <= 0 ) {
      SG_error("EVP_DigestSignInit rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EINVAL;
   }
   ...
   if( rc <= 0 ) {
      SG_error("EVP_DigestSignUpdate rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EINVAL;
   }
   ...
   if( rc <= 0 ) {
      SG_error("EVP_DigestSignFinal rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
      EVP_MD_CTX_destroy( mdctx );
      return -EINVAL;
   }
...
int md_encrypt( EVP_PKEY* sender_pkey, EVP_PKEY* receiver_pubkey, char const* in_data, size_t in_data_len, char** out_data, size_t* out_data_len ) {
   ...
   if( rc == 0 ) {
      SG_error("EVP_SealUpdate rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
   ...
   if( rc == 0 ) {
      SG_error("EVP_SealFinal rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
...
int md_decrypt( EVP_PKEY* sender_pubkey, EVP_PKEY* receiver_pkey, char const* in_data, size_t in_data_len, char** plaintext, size_t* plaintext_len ) {
   ...
   if( rc == 0 ) {
      SG_error("EVP_OpenInit rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
   ...
   if( rc == 0 ) {
      SG_error("EVP_OpenUpdate rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
   ...
   if( rc == 0 ) {
      SG_error("EVP_OpenFinal rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
...
int md_encrypt_symmetric_ex( unsigned char const* key, size_t key_len, unsigned char const* iv, size_t iv_len, char* data, size_t data_len, char** ciphertext, size_t* ciphertext_len ) {
   ...
   */
   ...
   if( rc == 0 ) {
      SG_error("EVP_EncryptInit_ex rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
   ...
   if( rc == 0 ) {
      ...
      SG_error("EVP_EncryptUpdate rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
   ...
   if( rc == 0 ) {
      SG_error("EVP_EncryptFinal_ex rc = %d, error_code = %ld\n", rc, ERR_get_error() );
      md_openssl_error();
